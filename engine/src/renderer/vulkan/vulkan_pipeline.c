...
)
{
    ...
    // Vertex input
    VkVertexInputBindingDescription binding_descrtption;
    binding_descrtption.binding = 0; // Binding index
    binding_descrtption.stride = sizeof(vertex_3d);
    binding_descrtption.inputRate = VK_VERTEX_INPUT_RATE_VERTEX; // Move to next data entry for each vertex

    // Attributes
    VkPipelineVertexInputStateCreateInfo vertex_input_info = { VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO };
    vertex_input_info.vertexBindingDescriptionCount = 1;
    vertex_input_info.pVertexBindingDescriptions = &binding_descrtption;
    ...
    KERROR("vkCreateGrpahicsPipelines failed with %s.", vulkan_result_string(result, true));
    return false;
}
